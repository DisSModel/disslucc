...
disslucc.allocation
---------------------
Faithful port of disslucc_continuous.components.allocation.raster.clue,
`main` branch (not `decoupling` -- that one was experimental). Raster
only. The algorithm (CLUE, Verburg et al. 1999 -- elasticity +
convergence + proportional correction + complementary class) was not
simplified: it's the same, cell by cell, vectorized with NumPy. What
got simpler was only what's around it (no Pipeline/Registry-class/
from_spec/TOML) -- see README.

Note: Allocation reads `<lu>_pot` directly from the shared backend
...
from ...schemas import AllocationSpec
...
class AllocationClueLike(SyncRasterModel):
    ...
    All per-cell operations are vectorized with NumPy -- no per-cell
    loop.
    """
    ...
    def setup(
        ...
        self.allocation_data = (
            list(allocation_data) if allocation_data is not None
            else [default for _ in range(len(land_use_types))]
        )
    ...
    def _compute_change(self, elasticity: list[float]) -> None:
        ...
        for lu_idx, lu in enumerate(self.land_use_types):
            ...
            new_val   = np.where(below_min, np.where(past >= min_val, min_val, past), new_val)
            above_max = new_val > max_val
            new_val   = np.where(above_max, np.where(past <= max_val, max_val, past), new_val)
    ...
    def _correct_cell_change(self) -> None:
        ...
        for i, lu in enumerate(lus):
            self.backend.arrays[lu] = np.where(flat_mask, vals[:, i], originals[lu]).reshape(shape)
    ...
    def _apply_complementar(self) -> None:
        ...
        if deficit.any():
            ...
            if eligible:
                stacked = np.stack(
                    [self.backend.get(lu).astype(np.float32) for lu in eligible], axis=0
                )
