...
disslucc.potential
--------------------
Faithful port of disslucc_continuous.components.potential.raster.linear
...
class PotentialLinearRegression(SyncRasterModel):
    """
    Potential via linear regression (Verburg et al. 1999).

    All driver and land use arrays must already be loaded into the
    RasterBackend. Array names must match the keys of
    RegressionSpec.betas.
    """
    ...
    def setup(
        ...
        # ensure the region array exists -- all cells in region 1 by default
    ...
    def _adapt_constants(self, r_number: int) -> None:
        for lu_idx, spec in enumerate(self.potential_data[r_number - 1]):
            ...
            if spec.is_log:
                if plus > 0:
                    spec.newconst -= math.log10(plus) * 0.01
                elif plus < 0:
                    spec.newconst += math.log10(-plus) * 0.01
            else:
